...
;(function ($, document, window){
	...
	attachEventsHandler = function(){
		...
		that.$element.find('.btn-wizard-previous:not(.disabled):not(.hidden)').on('click', function(e){
			e.stopPropagation();
			that.previous.call(that,true,e);
		});	
		
		that.$element.find('.btn-wizard-next').on('click', function (e) {
			e.stopPropagation();
			that.next.call(that, true, e);
			nextOrFinishButton = $(this);			
            if (nextOrFinishButton.hasClass("btn-success")) {
                nextOrFinishButton.trigger("activityFinalStepEvent", { isFinalstep: true });
            }
		});
		...
		that.$element.find('.steps > li').on('click', function (e) {
		    ...
		    //$("#stackContainer").show();
		    //$("#correctAnswerContainer").css("display", "block");
		    //$("#checkanswer").css("display", "block");
		    //$("#backtohome").css("display", "none");

		   // incrementCount = 0;
		    e.stopPropagation();
		    activeStepNo = parseInt($(this).attr('data-step'));
		    if (isFIBImplementation) setDefaultTextInputFocus();
	...
	checkStatus = function(){
		...
		that.$element.find('.btn-wizard-next').removeClass('final-step btn-success');
		that.$element.find('.btn-wizard-previous').removeClass('disabled hidden');
		if(that.currentStep == stepsItems.length){
			// we are in the last step
			that.$element.find('.btn-wizard-next').addClass('final-step btn-success');
		} else if(that.currentStep == 1){
			that.$element.find('.btn-wizard-previous').addClass('disabled hidden');
		}		
	...
	moveStep = function(step, direction, event, checkStep){		
		...
		if (triggerEnd) {
		    $(nextOrFinishButton).trigger("activityFinishedEvent", { isFinished: true });
	...
	methods = {
		init: function (element, options) {
			...
			if(opts.topButtons && stepsBar.length && !topActions.length){
				...
				html += '<span class="wizardnavbutton btn-wizard-previous"><span class="previous-text">' + opts.text.previous + '</span></span>';
				html += '<span class="wizardnavbutton btn-wizard-next"><span class="next-text">' + opts.text.next + '</span><span class="finished-text">' + opts.text.finished + '</span></span>';
				html += '</div></div>';
			...
			// add arrows to btn
			this.$element.find('.btn-wizard-previous').prepend('<i class="wiz-icon-arrow-left"></i>');
			this.$element.find('.btn-wizard-next').append('<i class="wiz-icon-arrow-right"></i>');
			this.$element.find('.btn-wizard-next > .finished-text').prepend('<i class="flaticon-champion3"></i>');
			...
			for(var i=0;i<stepsLi.length;i++){
				...
				step.empty().html(content + '<span class="wiz-icon-chevron-right colorA"></span>' + '<span class="step-index"><span class="label">'+ (i+1) +'</span></span>'+ '<span class="wiz-icon-chevron-right colorB"></span>');
	...
	$(document).ready(function () {
        // Modified by mahesh to accommodate application of plugin for every page in the activity set.
	    $(".wizard").each(function () {
	        $(this)[pluginName]({});
	    });

        // Original code
	    //var mySelector = document.querySelector('.wizard');
